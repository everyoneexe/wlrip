...
impl Player {
    ...
    /// Like is_valid but ignores dormant state. Used for caching so that
    /// temporarily-dormant players don't flicker out of the ESP list.
    pub fn is_alive(&self, cs2: &CS2) -> bool {
        if self.health(cs2) <= 0 {
            return false;
        }
    ...
    /// Whether the local player could realistically wallbang this target with
    /// the currently held weapon. Uses the physics BVH to measure how much
    /// solid lies between the eye and the target's bones, then runs a
    /// thickness-based penetration estimate. Surfaces are currently assumed to
    /// be `SURFACE_DEFAULT` (no material parsing yet).
    pub fn penetrable(&self, cs2: &CS2, local_player: &Player, min_damage: f32) -> bool {
        let Some(bvh) = &cs2.bvh else {
            return false;
        };

        let weapon_name = local_player.weapon_name(cs2);
        let base_damage = crate::cs2::penetration::weapon_base_damage(&weapon_name);
        let eye_pos = local_player.eye_position(cs2);
        ...
        CHECKED_BONES.iter().any(|bone| {
            let walls = bvh.cast_walls(eye_pos, self.bone_position(cs2, bone.u64()));
            crate::cs2::penetration::can_penetrate_walls(
                &walls,
                &weapon_name,
                base_damage,
                min_damage,
