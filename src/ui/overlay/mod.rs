...
/// Owns all state needed to render the ESP overlay on its own thread.
/// Decoupled from the winit `App` so it can run against the wayland
/// layer-shell surface independently.
...
impl OverlayRenderer {
    pub fn new(data: Arc<Mutex<Data>>, config: Config, grenades: GrenadeList) -> Self {
        Self {
            config,
            data,
            trails: HashMap::new(),
    ...
    /// Apply a config or grenade update pushed from the GUI thread.
    pub fn apply(&mut self, message: OverlayMessage) {
        match message {
            OverlayMessage::Config(config) => self.config = *config,
            OverlayMessage::Grenades(grenades) => self.grenades = *grenades,
        }
    }
